...
# I won't dig into TF interaction with the shell, feel free to explore the
# documentation
flags = tf.app.flags
...
flags.DEFINE_integer(
    'nb_process', 4, 'Number of parallel process to perform a HP search')

# fixed_params is a trick I use to be able to fix some parameters inside the model random function
# For example, one might want to explore different models fixing the
# learning rate, see the basic_model get_random_config function
flags.DEFINE_string('fixed_params', "{}",
...
flags.DEFINE_boolean(
    'best', False, 'Force to use the best known configuration')
...
flags.DEFINE_float(
    'nb_units', 20, 'Number of hidden units in Deep learning agents')
...
flags.DEFINE_integer('random_seed', random.randint(
    0, sys.maxsize), 'Value of random seed')
