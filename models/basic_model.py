...
class BasicAgent(object):
    # To build your model, you only to pass a "configuration" which is a
    # dictionary
    def __init__(self, config):
        # I like to keep the best HP found so far inside the model itself
        # This is a mechanism to load the best HP and override the
        # configuration
        if config['best']:
            config.update(self.get_best_config(config['env_name']))

        # I make a `deepcopy` of the configuration before using it
        # to avoid any potential mutation when I iterate asynchronously over
        # configurations
        ...
        # you store the random seed and actually use it in your TF graph
        # (tf.set_random_seed() for example)
    ...
    def get_best_config(self):
        # This function is here to be overriden completely.
        # It returns a dictionary used to update the initial configuration (see
        # __init__)
        return {}
    ...
    def get_random_config(fixed_params={}):
        ...
        # so they can independently generate random configuration of the
        # current model
        raise Exception(
            'The get_random_config function must be overriden by the agent')

    def build_graph(self, graph):
        raise Exception(
            'The build_graph function must be overriden by the agent')
    ...
    def learn_from_epoch(self):
        ...
        # to train globally
        raise Exception(
            'The learn_from_epoch function must be overriden by the agent')
    ...
    def save(self):
        ...
        global_step, episode_id = self.sess.run(
            [global_step_t, self.episode_id])
        if self.config['debug']:
            print('Saving to %s with global_step %d' %
                  (self.result_dir, global_step))
        self.saver.save(self.sess, self.result_dir +
                        '/agent-ep_' + str(episode_id), global_step)
